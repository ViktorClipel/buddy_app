...
// --- Nova Função para Calcular Tempo de Leitura ---
function calculateReadingTime(text) {
    const wordsPerMinute = 200; // Média de leitura em tela
    const words = text.trim().split(/\s+/).length; // Conta as palavras
    const time = (words / wordsPerMinute) * 60 * 1000; // Converte para milissegundos

    const minimumTime = 3000; // Mínimo de 3 segundos
    const maximumTime = 15000; // Máximo de 15 segundos

    // Retorna o tempo calculado, mas dentro dos limites mínimo e máximo
...
ipcMain.on('boot-continue', () => {
    if (windows.boot && !windows.boot.isDestroyed()) {
        windows.boot.close();
    }
...
function handleStreamChunk(chunk) {
    if (!streamState.isStreaming) {
        ...
        if (windows.speechBubble) {
            windows.speechBubble.webContents.send('stream-start');
        }
    }
...
ipcMain.on('resize-speech-bubble', (event, height) => {
    if (windows.speechBubble && !windows.speechBubble.isDestroyed()) {
        const [width] = windows.speechBubble.getSize();
        // Usa setBounds para redimensionar e garantir que a animação seja suave
        windows.speechBubble.setBounds({ width: width, height: Math.round(height) });
    }
});